...
void Battle::performAttack(Player& player, MapIni * map)
{
	string selectedCountry;
	string countryToAttack;
	vector<Country*> adjacentCountries;

	while (map->getCountry(selectedCountry) == NULL)
	{
		vector<Country*> ownedCountryList = player.getCountriesThatCanAttack();
		cout << "Which country will attack? " << endl;

		for (Country* country : ownedCountryList)
		{
			cout << country->getName() << " -> army size: " << country->getArmySize() << endl;
		}

		getline(cin, selectedCountry);
		Country* selected = map->getCountry(selectedCountry);
		adjacentCountries = map->getAdjacentCountriesOfOtherPlayers(selected, player);
	}


	while (map->getCountry(countryToAttack) == NULL)
	{
		cout << "Which adjacent country do you wish to attack? " << endl;
		for (Country* countryEnemy : adjacentCountries)
		{
			cout << countryEnemy->getName() << "- army power : " << countryEnemy->getArmySize() << endl;
		}

		getline(cin, countryToAttack);
	}

	Country *attackingCountry = map->getCountry(selectedCountry);
	Country *defendingCountry = map->getCountry(countryToAttack);
...
void Battle::executeBattle(BattlingEntity attacker, BattlingEntity defender)
{
	...
	if (defender.getCountry()->getArmySize() == 0 &&
		attacker.getCountry()->getArmySize() > 1)
...
void Battle::captureCountry(BattlingEntity attacker, BattlingEntity defender)
{
	...
	while (!validInput)
	{
		...
		cin >> armiesToMove;
		validInput = (armiesToMove >= attacker.getDice().getDices().size() && armiesToMove <= attacker.getCountry()->getArmySize());
	}
